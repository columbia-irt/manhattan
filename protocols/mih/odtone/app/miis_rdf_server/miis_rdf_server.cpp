...
private:
	 ...
	 */
	void run_query(const unsigned char *qry, unsigned char **results);	//yan - char* -> char**
...
void miis_rdf_server::event_handler(odtone::mih::message& msg, const boost::system::error_code& ec)
{
	...
	case odtone::mih::indication::get_information:
		...
		for(i = 0; i < query_list.size(); i++) {
			...
			// run query
			query_str = (unsigned char *) query_list[i]._data.c_str();	//yan - 0 -> i
			run_query(query_str, &result_str);	//yan
 ...
 * @param results Pointer to where to store the query results.
 */
void miis_rdf_server::run_query(const unsigned char *qry, unsigned char **results)	//yan - char *result -> char **result
{
	librdf_query *query;
	...
	*results = librdf_query_results_to_string(r, NULL, NULL);	//yan
	log_(0, *results);	//yan
}
