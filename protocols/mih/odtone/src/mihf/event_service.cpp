				...
				mih::mih_evt_list_enum event)
{
	...
	ODTONE_LOG(4, "(mies) get event type ", event,
		" with link type ", li.type,
		" and link address ", li.addr);	//yan
	for(it = _event_subscriptions.begin();
	    it != _event_subscriptions.end();
	    it++, i++) {
		ODTONE_LOG(4, "(mies) compare to event type ", it->event,
			" and link type ", it->link.type,
			" and link address ", it->link.addr);
		if ((it->event == event) && (it->link == li)) {
			ODTONE_LOG(3, "(mies) found registration of user: ",
			    it->user, " for event type ", event);	//yan
				       ...
				       mih::mih_evt_list_enum event)
{
	...
	*msg >> mih::indication()
	...
	//ODTONE_LOG(1,  (boost::get<odtone::mih::mac_addr>(li.addr)).address().c_str() );	//yan
