...
char hip_preferred_ifname[20] = {0};	//yan - 04/09/2012 - hard code max length of ifname
char hip_current_ifname[20] = {0};	//yan - 05/06/2012 - change routing table in readdress
volatile int prefer_changed = 0;	//yan - 04/20/2012
pthread_mutex_t pref_mutex = PTHREAD_MUTEX_INITIALIZER;	//yan - 04/11/2012
//pthread_mutex_t readdr_mutex = PTHREAD_MUTEX_INITIALIZER;	//yan - 04/19/2012
volatile int encrypt_off = 0;	//yan - 06/18/2012 - control encryption
...
char* get_router_str(const char* ifname);	//yan - 06/03/2012
...
int hip_netlink_open()
{
	...
	local.nl_groups = ( 0x40 | RTMGRP_LINK| RTMGRP_IPV4_IFADDR| RTMGRP_IPV6_IFADDR);
...
int select_preferred_address()
{
	...
	if ((HCNF.preferred.ss_family) || (preferred_iface_index != -1)) {
		//int old_family = (HIPA_SRC(hip_a))->addr.ss_family;	//yan - May 29, 2012 - IPv4/6
		for (l = my_addr_head; l; l=l->next) {
			...
			// yan - begin - May 29, 2012
			//log_(NORM, "%s %d %d %d\n", logaddr(SA(&l->addr)), l->addr.ss_family, AF_INET, AF_INET6);
#if (IP_VERSION == 6) 
				// IPv6 - May 29, 2012
			if ((l->addr.ss_family != AF_INET6) ||
	...
	if (!preferred_selected)
	{
		for (l = my_addr_head; l; l=l->next)
		{
			...
			if ((IS_LSI32(ip)) || ((ip & 0xFFFF) == 0xFEA9))
					continue;
			char com1[100]="netstat -ie | grep -B1 \"";
			char buf[100];
			sprintf(buf,"%s",logaddr(SA(&l->addr)));
			strcat(com1,buf);
			strcat(com1,"\" | head -n1 | awk '{print $1}'");
			//char *com2="";

			FILE *fp=popen(com1,"r");
			char dev_int[10];
			if (fgets(dev_int,sizeof(dev_int)-1,fp)!=NULL)
			{
				char com2[100]="netstat -rn | grep ^0.0.0.0 | awk '{print $8}'";
				FILE *fp2=popen(com2,"r");
				char dev_gate[10];
			    if (fgets(dev_gate,sizeof(dev_gate)-1,fp2)!=NULL)
				{
	    			//log_(NORM, "dev of the in is %sdev of the gate is %s\n",dev_int,dev_gate);

			    	if (strcmp(dev_int,dev_gate)==0)
			    	{
			    		preferred_selected = TRUE;

			    		log_(NORM, "%s selected as the ",logaddr(SA(&l->addr)));
			    		log_(NORM, "preferred address (gateway).\n");
...
int set_link_params(char *dev, int mtu)
{
	...
	//yan - after bringing up hip0, set up routing rules and tables
	int r;
	char cmd[100];
	hi_node* my_host_id = get_preferred_hi(my_hi_head);
	//addr_to_str(SA(&my_host_id->lsi), cmd, INET_ADDRSTRLEN);
...
int devname_to_index(char *dev, __u64 *mac)
{
	...
	while(!done) {
		...
		while (NLMSG_OK(h, status)) {
			...
			//if (tb[IFLA_IFNAME]) log_(NORM, "here2\n");	//yan
			//if (RTA_DATA(tb[IFLA_IFNAME]))	//yan
			//	log_(NORM, "here1%s\n", RTA_DATA(tb[IFLA_IFNAME]));	//yan
			//log_(NORM, "???%s/n", RTA_DATA(tb[IFLA_IFNAME]));	//yan
			if (tb[IFLA_IFNAME] && RTA_DATA(tb[IFLA_IFNAME]) && //tb[IFLA_ADDRESS] &&	//yan
			    (strcmp(dev, RTA_DATA(tb[IFLA_IFNAME])) == 0)) {
				if (mac && tb[IFLA_ADDRESS]) {	//yan
	...
	for (msg = (struct nlmsghdr*)data; NLMSG_OK(msg, (__u32)length);
		...
		case RTM_DELADDR:
			...
			if (is_add)
			{
				...
				for (l2 = my_addr_head; l2; l2=l2->next)
				{
					if (!(l2->status == DELETED)&&!strcmp(logaddr(SA(&l2->addr)),logaddr(addr)))
					{
						log_(NORM, "The same %d", l2->status);
						thesame=TRUE;
					}
				}			
			}
		...
		case RTM_NEWROUTE:
		case RTM_DELROUTE:
			log_(NORM, "route change\n");
			//sleep(1);	//Yan-02/26/2012
    ...
    int if_index)
{
	...
	log_(NORM, "Lock in readdress_association");
	//pthread_mutex_lock(&readdr_mutex);	//yan - Apr 19 2012
	...
	//pthread_mutex_unlock(&readdr_mutex);	//yan - Apr 19 2012
}
    ...
    struct sockaddr *newdstaddr, int if_index)
{
	...
	//yan - May 6, 2012 - return if no preferred interface set
        if (hip_preferred_ifname[0] == 0) {
                return;
        }
        //yan - end
