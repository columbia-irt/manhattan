			     ...
			     size_t optvlen)
{
	...
	dbg("A packet sent through interface %d\n", oif);	//yan
		  ...
		  struct iovec *opts, size_t optslen)
{
	...
	dbg("A router solicitation is sent through %d\n", ifindex);	//yan
...
int ndisc_send_ns(int ifindex, const struct in6_addr *target)
{
	...
	dbg("A neighbor solicitation is sent through %d\n", ifindex);     //yan
		  ...
		  const struct in6_addr *target, uint32_t flags)
{
	...
	dbg("A neighbor advertisement is sent through %d\n", ifindex);     //yan
