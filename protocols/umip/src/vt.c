...
//yan-03/13/2012
static int vt_cmd_changepref(const struct vt_handle *vh, const char *str)
{
	char temp[20];
	int i;
	...
	temp[i] = 0;
	if (pref_change(temp) < 0) {	//from movement.c
		fprintf(vh->vh_stream, "Already on %s\n", str);
	} else {
		fprintf(vh->vh_stream, "Interface changed\n");
	}
	return 0;
}
//yan-end
...
//yan-03/13/2012
static struct vt_cmd_entry vt_cmd_pref = {
	.cmd = "pref",
	.parser = vt_cmd_changepref,
};
//yan-end
...
static int vt_cmd_sys_init(void)
{
	...
	//yan-03/13/2012
	ret = vt_cmd_add_root(&vt_cmd_pref);
	if (ret < 0)
		return ret;
	//yan-end
