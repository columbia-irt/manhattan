					...
					struct md_coa *coa)
{
	...
	if (iface->preference > 1) return;	//yan - for mih
	...
	MDBG("strategy %d type %d iface %s (%d) " 	//yan- 2 to 1	
	      "CoA %x:%x:%x:%x:%x:%x:%x:%x\n",
...
static void md_flush_inet6_iface(struct md_inet6_iface *iface)
{
	...
	if (!iface->is_tunnel) {
		...
		list_for_each_safe(l, n, &iface->backup_rtrs) {
			...
			//MDBG("going to expire router from md_flush_inet6_iface\n");	//yan
...
static int process_del_addr(struct ifaddrmsg *ifa, struct rtattr **rta_tb)
{
	...
	MDBG("deleted address %x:%x:%x:%x:%x:%x:%x:%x on iface %d\n", 
	      NIP6ADDR(addr), ifa->ifa_index);	//yan 3->1
...
{
	MDBG("discover link on iface %s (%d)\n", iface->name, iface->ifindex);	//yan 1->2

	//MDBG("rs: %d, max: %d\n", iface->router_solicits, iface->devconf[DEVCONF_RTR_SOLICITS]); //yan
				     ...
				     struct nlmsghdr *n, void *arg)
{
	...
	route_add(ifindex, RT_TABLE_MAIN, RTPROT_STATIC, r->rtm_flags,		//yan - stop it from controling routing table
...
{
	MDBG2("link up on iface %s (%d)\n", iface->name, iface->ifindex);	//yan - 2to1
	...
	if (!iface->is_tunnel) {
		...
		if (link_changed) {
			...
			list_for_each_safe(l, n, &iface->backup_rtrs) {
				struct md_router *rtr;
				rtr = list_entry(l, struct md_router, list);
				//MDBG("going to expire router from __md_new_link\n");     //yan
				md_expire_router(iface, rtr, NULL);
			}
		}
		md_flush_router_list(&iface->expired_rtrs);
...
md_create_inet6_iface(struct ifinfomsg *ifi, struct rtattr **rta_tb)
{
	...
	if ((iface = malloc(sizeof(struct md_inet6_iface))) != NULL) {
		...
		//if (iface->devconf[DEVCONF_RTR_SOLICITS] <= 0)	//yan - why? 250
			iface->devconf[DEVCONF_RTR_SOLICITS] = conf_default_rs;
		//if (iface->devconf[DEVCONF_RTR_SOLICIT_INTERVAL] <= 0)	//yan - why?
...
static void md_probe_router(struct md_router *rtr)
{
	if (md_is_link_up(rtr->iface)) {
		...
		MDBG("preform NUD for router "		//yan - 2to1
		      "%x:%x:%x:%x:%x:%x:%x:%x on interface %s (%d)\n", 
...
static void md_router_timeout(struct md_router *rtr)
{
	...
	MDBG("expire router %x:%x:%x:%x:%x:%x:%x:%x "	//yan - 2to1
	...
	if (rtr->used) {
		if (list_empty(&iface->backup_rtrs) ||
			...
			//if (rtr->iface->preference > 1)	{ //yan - for mih
			//	__md_free_router(rtr);
			//}
...
static void md_update_router_stats(struct md_router *rtr)
{
	...
	MDBG2("adding default route via %x:%x:%x:%x:%x:%x:%x:%x with metric"	//yan-2 to 1
	      " %d\n", NIP6ADDR(&rtr->lladdr), metric);

	if (iface_pref == 1)	//yan - adding default entry for particular interface
 				     ...
 				     struct md_router *old)
{
	...
	if (old) {
		MDBG("going to expire router from md_change_default_router\n");     //yan
...
md_check_backup_routers(struct md_inet6_iface *iface, struct md_router *new)
{
	...
	list_for_each_safe(list, n, &iface->backup_rtrs) {
		...
		if (!md_router_cmp(new, old)) {
			md_update_router(new, old);
			if (!tsisset(old->lifetime)) {
				MDBG("going to expire router from md_check_backup_routers\n");     //yan
...
md_check_default_router(struct md_inet6_iface *iface, struct md_router *new)
{
	...
	if (!tsisset(old->lifetime)) {
		MDBG("going to expire router from md_check_default_router\n");     //yan
		md_expire_router(iface, old, NULL);
		__md_discover_router(iface);
		       ...
		       const struct in6_addr *daddr, int iif, int hoplimit)
{
	...
	if ((iface = md_get_inet6_iface(&ifaces, iif)) == NULL ||
	    ...
	    rtr->probed == 0 || rtr->hwalen < 0 || //iface->preference > 1 ||	//yan
		       ...
		       const struct in6_addr *daddr, int iif, int hoplimit)
{
	...
	if ((iface = md_get_inet6_iface(&ifaces, iif)) != NULL &&
		...
		if (//iface->preference == 1 &&	//yan
		    !md_check_expired_routers(iface, new) &&
...
//yan-start-block dad
/*
static void md_recv_ns(const struct icmp6_hdr *ih, ssize_t len,
			const struct in6_addr *saddr,
                       const struct in6_addr *daddr, int iif, int hoplimit)
{
        //struct nd_router_advert *ra = (struct nd_router_advert *)ih;
        ...
        MDBG("received NS from %x:%x:%x:%x:%x:%x:%x:%x on iface %d\n",         //yan-2 to 1
              NIP6ADDR(saddr), iif);

        pthread_mutex_lock(&iface_lock);
        if ((iface = md_get_inet6_iface(&ifaces, iif)) != NULL &&
            (!iface->is_tunnel) && (iface->preference == 1) &&          //yan
            (new = md_create_router(iface, saddr, ra, len)) != NULL) {
...
static struct icmp6_handler md_ns_handler = {
        .recv = md_recv_ns,
};
*/
//yan-end
...
int md_start(void)
{
	...
	//icmp6_handler_reg(ND_NEIGHBOR_SOLICIT, &md_ns_handler);		//yan
...
{
	rtnl_close(&md_rth);
	//icmp6_handler_dereg(ND_NEIGHBOR_SOLICIT, &md_ns_handler);	//yan
...
int pref_change(const char *ifname) {
	...
	list_for_each(l, &ifaces) {
		...
		MDBG("Changing preference of interface %s from %d to %d\n",
					iface->name, iface->preference, pref);
		//if (pref > 1 && iface->preference == 1 && !list_empty(&iface->default_rtr)) {
		//	iface->preference = pref;
		//}
		iface->preference = pref;
	}

	if (newif) {	//there exists a new interface
		//md_link_up(newif);
		if (oldif) {	//delete the old prefix from the routing table
			if (oldif->is_tunnel) {
				route_del(oldif->ifindex, RT_TABLE_MAIN, 0,
					&in6addr_any, 0, &in6addr_any, 0, 0);
			} else if ((rtr = md_get_first_router(&oldif->default_rtr)) != NULL && rtr->used) {
				//__md_free_router(rtr);
				route_del(rtr->ifindex, RT_TABLE_MAIN, 0,
					&in6addr_any, 0, &in6addr_any, 0, &rtr->lladdr);
		...
		if (newif->is_tunnel) {
			...
			ndisc_send_rs(newif->ifindex, &in6addr_all_nodes_mc, NULL, 0);
		}
...
void pref_change() {
	struct list_head *l;
	struct md_inet6_iface *bestif = NULL;

	FILE* fp;
	char ifname[20];
	int pref = 0;

	fp = fopen("/home/mika/mipv6/preferences", "r");
	if (fp == NULL) {
		return;
	}

	//pref = 1;
	//while (fgets(ifname, 20, fp) != NULL) {
	if (fgets(ifname, 20, fp) != NULL) {
		int i;
	...
	if (bestif) {
		//trigger handoff - DAD failed
		//__md_trigger_movement_event(ME_LINK_UP, 0, bestif, NULL);
		ndisc_send_rs(bestif->ifindex, &in6addr_all_routers_mc, NULL, 0);
	}
