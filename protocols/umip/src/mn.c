...
//mn_send_home_bu: do_handoff / mn_change_ha
//do_handoff: mn_update_home_prefix / coa_updated / mn_move
...
//static int preferred_ifindex = -1;	//yan - 06/25/2012
...
static int mv_hoa(struct ifaddrmsg *ifa, struct rtattr *rta_tb[], void *arg)
{
	...
	hoa->iif = mha->if_next;
	addr_del(&hoa->addr, ifa->ifa_prefixlen, ifa->ifa_index);	//yan - to work with hip
	return 0;
}
			  ...
			  const struct nd_opt_prefix_info *pinfo)
{
	...
	//MDBG("mn_update_home_prefix is called!\n");	//yan - 06/25/2012
...
static int mn_make_ho_verdict(const struct movement_event *me,
			      struct home_addr_info *hai, //hai is modified here
			      ...
			      struct md_coa **next_coa)
{
	...
	case ME_COA_LFT_INC:
		...
		if (old_iface != me->iface)
			break;		//yan - handoff when lifetime change
			      ...
			      const struct movement_event *event)
{
	...
	case MN_HO_IGNORE:
		MDBG("MN_HO_IGNORE\n");
		break;
	case MN_HO_INVALIDATE:
		MDBG("MN_HO_INVALIDATE\n");
		break;
	case MN_HO_PROCEED:
		MDBG("MN_HO_PROCEED\n");
		break;
	case MN_HO_RETURN_HOME:
		MDBG("MN_HO_RETURN_HOME\n");
		break;
	case MN_HO_CHECK_LIFETIME:
		MDBG("MN_HO_CHECK_LIFETIME\n");
		break;
	case MN_HO_REESTABLISH:
		MDBG("MN_HO_REESTABLISH\n");
		break;
	default:
		MDBG("MN_HO_NONE\n");
		break;
...
int mn_movement_event(struct movement_event *event)
{
	...
	if (!pending_bas) {
		list_for_each(lh, &home_addr_list) {
			hai = list_entry(lh, struct home_addr_info, list);
			//MDBG("%d %d\n", hai->at_home, positive_ho_verdict(hai->verdict));	//yan
			if (!hai->at_home && 
